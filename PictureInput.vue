<template>
  <div ref="container" id="picture-input" class="picture-input">
    ...
    <div v-else-if="supportsPreview">
      ...
      <button v-if="imageSelected && !hideChangeButton" @click.prevent="selectImage" :class="buttonClass" type="button" v-html='strings.change'></button>
      <button v-if="imageSelected && removable" @click.prevent="removeImage" :class="removeButtonClass" type="button" v-html='strings.remove'></button>
      <button v-if="imageSelected && toggleAspectRatio && width !== height" @click.prevent="rotateImage" :class="aspectButtonClass" type="button" v-html='strings.aspect'></button>
    </div>
    <div v-else>
      ...
      <div v-else>
        ...
        <button v-if="removable" @click.prevent="removeImage" :class="removeButtonClass" type="button" v-html='strings.remove'></button>
      </div>
    </div>
  ...
  methods: {
    ...
    preloadImage (source, options) {
      ...
      .then(({ imageBlob, mimeType }) => {
        ...
        let mediaType = options.mediaType || ('image/' + mimeType || (options.fileType || fileName.split('.').slice(-1)[0].split('?')[0]))
